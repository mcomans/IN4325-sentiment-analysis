...
def run(feature_vectors, labels):
    ...
    classify_ova(X_train, X_test, y_train, y_test)
...
for author_name in subjective_sentences_files:
    ...
    run(feature_vectors, read_labels(three_class_labels_file))

    print("With four class labels")
    run(feature_vectors, read_labels(four_class_labels_file))
